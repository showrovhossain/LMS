...
def validate_bio_length(value):
    words = value.split()
    if len(words) > 100:
        raise ValidationError("The bio can have a maximum of 100 words.")
...
class Author(models.Model):
    ...
    author_bio = models.CharField(max_length=500, null=True, blank=True, validators=[validate_bio_length])
...
class Payment(models.Model):
    ...
    user_course = models.ForeignKey('UserCourse', on_delete=models.CASCADE,null=True)  # Update this based on your course model
    ...
    def __str__(self):
        return f"Payment {self.payment_id} by {self.user.username} for Order {self.order.id}"
